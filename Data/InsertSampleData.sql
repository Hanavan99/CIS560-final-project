...
INSERT INTO Tasks.Tasks ([Name], [Description], OwnerUserID, TaskStateID, UserGroupID, DueDate, StartDate, CompletionDate) values 
	(N'Create new user', N'Create an account on the network for new hire', (SELECT UserID FROM Users.Users WHERE [Name] = N'esandal2'), (SELECT TaskStateID FROM Tasks.TaskStates WHERE [Name] = N'In Progress'), (SELECT UserGroupID FROM Users.UserGroups WHERE [Name] = N'Networking and Security'), '2019-11-25', '2019-11-18', '2019-11-25');
INSERT INTO Tasks.Tasks ([Name], [Description], OwnerUserID, TaskStateID, UserGroupID, DueDate, StartDate, CompletionDate) values 
